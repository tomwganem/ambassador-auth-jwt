...
func TestDecode(t *testing.T) {
	...
	// // signeropts := signeroptions.WithHeader(header, keyid)
	// // signeropts := signeroptions.WithType("JWT")
	// // signerkey := jose.SigningKey{Algorithm: jose.RS256, Key: rsaPrivateKey}
	// // sig, err := jose.NewSigner(signerkey, signeropts)
	// sig, err := jose.NewSigner(jose.SigningKey{Algorithm: jose.RS256, Key: rsaPrivateKey}, nil)
	// // if err != nil {
	// // 	panic(err)
	// // }
	...
	// fmt.Println(json.Marshal(builder))
	// key := jose.JSONWebKey{
	// 	Algorithm: "RS256",
	...
	// // a, b, err := Decode(builder, keyset, issuer)
	// // fmt.Println(a)
	// fmt.Println(keyset)
	// assert.Nil(t, err)
}
